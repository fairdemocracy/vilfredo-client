...
$(function () {
	...
	$('<div id="header-links">' +
		...
		'<a href="http://www.amazon.com/wishlist/2OEDAIL14M64W/ref=cm_wl_rlist_go" target="_blank">Amazon Wishlist</a>.</p></div>' +
	...
	if ($.fn.bookmark) {
		$('#bookmark').bookmark({compact: true, popup: true, icons: 'img/bookmarks.png',
			...
			sites: ['bitly', 'blogger', 'delicious', 'digg', 'diigo', 'dzone', 'facebook',
			'google', 'googlereader', 'hotmail', 'linkedin', 'mixx', 'myspace', 'reddit', 'sphinn',
			'squidoo', 'stumbleupon', 'technorati', 'tumblr', 'twitter', 'yahoo', 'yahoobuzz']});
	}
	// Ratings
...
function showRating(name, id, prefix) {
	var rating = {backgroundpos: [0.0, 0], bookmark: [4.0, 9],
		...
		svg: [4.5, 60], themes: [4.0, 2],
		timeentry: [4.0, 42]}[name];
	var html = '';
	if (rating) {
		html = (prefix || '') + '<span>';
		for (var i = 0; i < 5; i++) {
